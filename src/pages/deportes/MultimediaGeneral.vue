<template>
  <q-page class="flex flex-center view_hijos_socios view_fitness">
    <Menudeportes :currentItem="'/deportes/multimedia'"/>
    <div class="q-pt-xl all_width gris_home">
        ...
        <div class="filters">
            <ul class="wrp_actions_center_peluqueria">
                <li><a href="#" class="anchor anchor-active" @click="getMultimedia($event, 'todos')">Todos</a></li>
                <li><a href="#" class="anchor" @click="getMultimedia($event, 'golf')">Golf</a></li>
                <li><a href="#" class="anchor" @click="getMultimedia($event, 'hockey')">Hockey</a></li>
                <li><a href="#" class="anchor" @click="getMultimedia($event, 'rugby')">Rugby</a></li>
                <li><a href="#" class="anchor" @click="getMultimedia($event, 'natacion')">Natación</a></li>
                <li><a href="#" class="anchor" @click="getMultimedia($event, 'squash')">Squash</a></li>
                <li><a href="#" class="anchor" @click="getMultimedia($event, 'paddle')">Paddle</a></li>
                <li><a href="#" class="anchor" @click="getMultimedia($event, 'tennis')">Tennis</a></li>
                <li><a href="#" class="anchor" @click="getMultimedia($event, 'Futbol')">Fútbol</a></li>
            </ul>
        </div>
    </div>
                ...
                <q-carousel-slide class="column no-wrap" :name="key" v-for="(item, key) in slidersContentVideos" :key="key">
                    <div class="row fit justify-between items-center q-gutter-xs q-col-gutter no-wrap new_multi">
                        <table class="item_cien">
                           <tr>
                                <td class="first" rowspan="0" v-for="(subItem, subKey) in item" :key="subKey">
                                    <a href="#" @click="openVideo($event, subItem)">
                                      <img :src="urlSite + subItem.field_portada_multimedia" />
                                      <div class="info_bottom text-center">
                                          <p class="desc">{{ subItem.title }}</p>
                                          <span v-html="subItem.body"></span>
                                      </div>
                                    </a>
                                </td>
                            </tr>
...
export default {
  ...
  methods: {
    ...
    getMultimedia (e, path) {
      e.preventDefault()
      this.addCurrentClass(e)
      if (path === 'todos') {
        return this.getMultimediaTodas()
      }
      var _this = this
      configServices.loadData(this, '/multimedia-secciones/' + path + '/json', {
        callBack: (data) => {
          data.map((item, key) => {
